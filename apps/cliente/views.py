...
def view_historial_anual(request):
	...
	return render_to_response("historial/anual.html",ctx,
		context_instance = RequestContext(request))

def view_historial_mensual(request):
	lista_cliente = Cliente.objects.filter(status=True) #para filtrar clientes activos
	print lista_cliente, 'lista'
	ctx = {'lista_cliente':lista_cliente}
	return render_to_response("historial/mensual.html",ctx,
		context_instance = RequestContext(request))
...
def view_lista_cliente_add(request):
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			if c.tipo_usuario.id == 3 or c.tipo_usuario.id == 4 or c.tipo_usuario.id == 5 or c.tipo_usuario.id == 6:
				c.iva = True
			c.status= True
			c.save()
